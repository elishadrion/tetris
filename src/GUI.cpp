...
void GUI::window_grid_GUI(){
	...
	*/
    ...
    for(i=0; i < 20 ; i++){
   	...
   	mvprintw(1, 54, "Next");
    BOX_NEXT_TETRIMINOS = subwin(stdscr,4,12,2,50);
...
void GUI::update_next_tetriminos_GUI(Grid * grid){
	/*
	Cette fonction efface l'affiche du prochain tétriminos.
	*/
	for(int i=0; i<4; i++){

		int y = grid->get_tetriminos()->get_coord_Y_of_block(i);
		int x = grid->get_tetriminos()->get_coord_X_of_block(i);
		attron(A_BOLD | COLOR_PAIR(8));
		mvprintw( 4+y,46+x*2 , "  ");
	}
