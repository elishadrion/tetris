...
\newglossaryentry{carte}{
  name=Carte,
  description= asdfasdf Wizard Poker
}

\newglossaryentry{app}{
  name=Application/système,
  description=Le jeu Wizard Poker à développer.
}
...
\newglossaryentry{sort} {
  name=Sort,
  description=Carte ayant un côut en énergie et un effet spécial.
}

\newglossaryentry{crea} {
  name=Créature,
  description={Carte ayant un côut en énergie, valeur d'attaque et des
  points de vie. Aussi (optionellement)  un effet spécial.}
}

\newglossaryentry{energie} {
  name=Énergie,
  description={Permet de jouer des cartes. Quantité, entier.}
}
...
Une fois son \gls{deck} créé le joueur peut jouer une partie contre quelqu'un d'autre.  Avant de jouer contre une personne connectée choisie arbitrairement par le programme, le joueur sera invité à choisir son \gls{deck}.  Quant au mode de jeu, il s'agit d'un jeu de cartes au tour par tour.  Ce qui signifie que chaque utilisateur joue à son tour.  Ils ne jouent donc jamais en même temps.  Lorsque c'est son tour le joueur a le choix entre plusieurs actions. Il peut:
\begin{itemize}
...
\end{itemize}
% TO DO : expliquer les contraintes: nombre de cartes, point de vie, ...
