...
\index{créature}s pouvant attaquer et des cartes de type
\index{sort} qui lanceront des événements spéciaux.
...
\index{deck} avant d'affronter son premier adversaire.
...
\begin{itemize}
 \item[\textbullet] poser une carte \index{sort} ;
...
Plusieurs actions peuvent être effectuées sur les \index{deck}s telles
...
actuellement connectés. Pour cela, un tchat sera mis à la disposition
du joueur.  Le joueur pourra donc communiquer avec ses amis par
message à tout moment. Cependant un joueur ne peut communiquer qu'avec
