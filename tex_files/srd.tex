...
\newglossaryentry{deck}{
  name=Deck,
  description=est un ensemble de 20 cartes.
}
...
\newglossaryentry{defausser} {
  name=Défausser,
  description={Action de se débarrasser d'une carte qui sera elle-même placée dans une défausse en vue d'être récupérée par le joueur qui l'a possède.}
}
...
Il existe deux types de cartes dans le jeu. D'une part, on retouve les cartes Sorts qui sont des cartes à effets. Cepedant, dès qu'une carte sort aura été jouée, elle sera directement défaussée et son effet sera immédiat (dans la majorité des cas). D'autre part, on retrouve les cartes monstres qui sont des cartes devant être posées sur le plateau par le joueur. Pour pouvoir attaquer avec les monstres, ces derniers doivent être sur le terrain depuis plus d'un tour au minimum. Certains monstres peuvent aussi avoir un voire plusieurs effets spéciaux.

\medbreak

Avec cette \gls{collection} le joueur pourra créer un, voire, des \gls{deck}s.  Un \gls{deck} est composé d'exactement 20 cartes.  De plus, il peut y avoir au maximum deux fois la même carte dans un \gls{deck} mais seulement si le joueur possède au moins deux exemplaire de la-dite carte dans sa \gls{collection}.
...
Le Wizard Poker étant un jeu en ligne, il permet donc de mettre les joueurs en contact. Premièrement, chaque joueur a la possibilité de gérer une liste d'amis. Un joueur pourra donc ajouter et retirer des amis dans sa liste. Mais il pourra également jouer des matchs avec un de ses amis. Deuxièmement, le Wizard Poker propose un système de Tchat permettant à deux joueurs étant amis de s'échanger des messages.  Ce tchat est disponible à tout moment. 
...
Lors de chaque début de partie, chaque joueur commence avec 20 points de vie, 0 point d'énergie et 5 cartes en main. À chaque début de tour, le joueur en question gagnera un point d'énergie supplémentaire jusqu'à atteindre un maximum de 10 points. 
...
Les exigences non fonctionnelles, n'étant, pour la plupart, pas indispensables à la mise en oeuvre de l'application, ne seront pas mises en oeuvre en priorité.
